...
class ContrastiveDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Union[Tuple[torch.Tensor, int], torch.Tensor]:
        ...
        # 다른 이미지 인덱스 랜덤 선택
        rand_indx = random.randint(0, len(self.image_paths) - 1)
        while rand_indx == index:
            rand_indx = random.randint(0, len(self.image_paths) - 1)

        img_path2 = os.path.join(self.root_dir, self.image_paths[rand_indx])
        ...
        if self.is_inference:
            ...
            target = self.targets[index]  # 해당 이미지의 레이블
            return image, image2, target  # 변환된 이미지와 레이블을 튜플 형태로 반환합니다.
